    ...
    MicroBlock &block)   ///< block to build in/out
{
    ...
    // is this the very first microblock then previous should point to 0
    ...
    for (uint8_t delegate = 0; delegate < NUM_DELEGATES; ++delegate)
    {
        ...
        if (false == _store.batch_tip_get(delegate, hash))
        {
            ...
            // done with this delegate's blockchain if reached the previous tip or NULL
    ...
    Epoch epoch;
    block._epoch_number = 0;
    if (!_store.epoch_tip_get(hash) && !_store.epoch_get(hash, epoch))
    {
    ...
    block._micro_block_number = (!no_previous) ? previous._micro_block_number + 1 : 0; // 0 based
