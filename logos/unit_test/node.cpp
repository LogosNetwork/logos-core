...
TEST (node, block_store_path_failure)
{
    ...
    auto path (logos::unique_path ());
    logos::logging logging;
    logging.init (path);
    logos::work_pool work (std::numeric_limits<unsigned>::max (), nullptr);
    auto node (std::make_shared<logos::node> (init, *service, 0, path, alarm, logging, work));
...
TEST (node, state_canaries)
{
    ...
    auto node (std::make_shared<logos::node> (init, *service, path, alarm, config, work));
    ASSERT_EQ (logos::block_hash (10), node->ledger.state_block_parse_canary);
    ASSERT_EQ (logos::block_hash (20), node->ledger.state_block_generate_canary);
    node->stop ();
}
...
{
    logos::system system (24000, 1);
    system.wallet (0)->insert_adhoc (logos::test_genesis_key.prv);
    logos::transaction transaction (system.nodes[0]->store.environment, nullptr, true);
    ASSERT_EQ (std::numeric_limits<logos::uint128_t>::max (), system.nodes[0]->ledger.account_balance (transaction, logos::test_genesis_key.pub));
}

TEST (node, representative)
{
    logos::system system (24000, 1);
    auto block1 (system.nodes[0]->representative (logos::test_genesis_key.pub));
    {
        logos::transaction transaction (system.nodes[0]->store.environment, nullptr, false);
...
TEST (node, send_unkeyed)
{
    ...
    logos::keypair key2;
    system.wallet (0)->insert_adhoc (logos::test_genesis_key.prv);
    system.wallet (0)->store.password.value_set (logos::keypair ().prv);
    ASSERT_EQ (nullptr, system.wallet (0)->send_action (logos::test_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
}
...
TEST (node, send_self)
{
    ...
    ASSERT_NE (nullptr, system.wallet (0)->send_action (logos::test_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
    auto iterations (0);
    ...
    ASSERT_EQ (std::numeric_limits<logos::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), system.nodes[0]->balance (logos::test_genesis_key.pub));
}

TEST (node, send_single)
{
    logos::system system (24000, 2);
    logos::keypair key2;
    system.wallet (0)->insert_adhoc (logos::test_genesis_key.prv);
...
TEST (node, send_single_observing_peer)
{
    ...
    ASSERT_TRUE (system.nodes[0]->balance (key2.pub).is_zero ());
    auto iterations (0);
    while (std::any_of (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<logos::node> const & node_a) { return node_a->balance (key2.pub).is_zero (); }))
    {
        system.poll ();
        ++iterations;
        ASSERT_LT (iterations, 200);
...
TEST (node, send_out_of_order)
{
    ...
    logos::send_block send1 (genesis.hash (), key2.pub, std::numeric_limits<logos::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number (), logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.work.generate (genesis.hash ()));
    logos::send_block send2 (send1.hash (), key2.pub, std::numeric_limits<logos::uint128_t>::max () - system.nodes[0]->config.receive_minimum.number () * 2, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.work.generate (send1.hash ()));
    system.nodes[0]->process_active (std::unique_ptr<logos::block> (new logos::send_block (send2)));
    system.nodes[0]->process_active (std::unique_ptr<logos::block> (new logos::send_block (send1)));
    auto iterations (0);
    while (std::any_of (system.nodes.begin (), system.nodes.end (), [&](std::shared_ptr<logos::node> const & node_a) { return node_a->balance (logos::test_genesis_key.pub) != logos::genesis_amount - system.nodes[0]->config.receive_minimum.number () * 2; }))
    {
        system.poll ();
        ++iterations;
        ASSERT_LT (iterations, 200);
...
TEST (node, quick_confirm)
{
    logos::system system (24000, 1);
    logos::keypair key;
    logos::block_hash previous (system.nodes[0]->latest (logos::test_genesis_key.pub));
    system.wallet (0)->insert_adhoc (key.prv);
    system.wallet (0)->insert_adhoc (logos::test_genesis_key.prv);
    auto send (std::make_shared<logos::send_block> (previous, key.pub, system.nodes[0]->delta () + 1, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.work.generate (previous)));
...
TEST (node, node_receive_quorum)
{
    ...
    logos::block_hash previous (system.nodes[0]->latest (logos::test_genesis_key.pub));
    system.wallet (0)->insert_adhoc (key.prv);
    auto send (std::make_shared<logos::send_block> (previous, key.pub, logos::genesis_amount - logos::Gxrb_ratio, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.work.generate (previous)));
    system.nodes[0]->process_active (send);
    auto iterations (0);
    ...
    while (!done)
    {
        ...
        done = info->announcements > logos::active_transactions::announcement_min;
        system.poll ();
        ++iterations;
        ASSERT_LT (iterations, 200);
    }
...
TEST (node, auto_bootstrap)
{
    ...
    logos::node_init init1;
    auto node1 (std::make_shared<logos::node> (init1, system.service, 24001, logos::unique_path (), system.alarm, system.logging, system.work));
...
TEST (node, receive_gap)
{
    ...
    auto block (std::make_shared<logos::send_block> (5, 1, 2, logos::keypair ().prv, 4, 0));
    node1.work_generate_blocking (*block);
    logos::confirm_req message;
    message.block = block;
...
{
    logos::system system (24000, 1);
    std::array<logos::endpoint, 8> endpoints;
    endpoints.fill (logos::endpoint (boost::asio::ip::address_v6::loopback (), 24000));
    endpoints[0] = logos::endpoint (boost::asio::ip::address_v6::loopback (), 24001);
...
TEST (node, search_pending_multiple)
{
    ...
    ASSERT_NE (nullptr, system.wallet (0)->send_action (logos::test_genesis_key.pub, key3.pub, system.nodes[0]->config.receive_minimum.number ()));
    auto iterations1 (0);
...
TEST (node, unlock_search)
{
    logos::system system (24000, 1);
    logos::keypair key2;
    logos::uint128_t balance (system.nodes[0]->balance (logos::test_genesis_key.pub));
    {
        logos::transaction transaction (system.wallet (0)->store.environment, nullptr, true);
        system.wallet (0)->store.rekey (transaction, "");
    }
    system.wallet (0)->insert_adhoc (logos::test_genesis_key.prv);
    ASSERT_NE (nullptr, system.wallet (0)->send_action (logos::test_genesis_key.pub, key2.pub, system.nodes[0]->config.receive_minimum.number ()));
    auto iterations1 (0);
    while (system.nodes[0]->balance (logos::test_genesis_key.pub) == balance)
    {
        system.poll ();
        ++iterations1;
        ASSERT_LT (iterations1, 200);
...
TEST (node, connect_after_junk)
{
    ...
    auto iterations1 (0);
    while (system.nodes[0]->stats.count (logos::stat::type::error) == 0)
    {
        system.poll ();
        ++iterations1;
        ASSERT_LT (iterations1, 200);
...
{
    auto path1 (logos::unique_path ());
    auto path2 (logos::unique_path ());
    logos::logging logging1;
    logging1.init (path1);
    logos::logging logging2;
    logging2.init (path2);
...
{
    logos::system system (24000, 1);
    auto price1 (system.nodes[0]->price (logos::Gxrb_ratio, 1));
    ASSERT_EQ (logos::node::price_max * 100.0, price1);
    auto price2 (system.nodes[0]->price (logos::Gxrb_ratio * int(logos::node::free_cutoff + 1), 1));
    ASSERT_EQ (0, price2);
    auto price3 (system.nodes[0]->price (logos::Gxrb_ratio * int(logos::node::free_cutoff + 2) / 2, 1));
    ASSERT_EQ (logos::node::price_max * 100.0 / 2, price3);
    auto price4 (system.nodes[0]->price (logos::Gxrb_ratio * int(logos::node::free_cutoff) * 2, 1));
    ASSERT_EQ (0, price4);
}
...
TEST (node_config, serialization)
{
    ...
    logos::logging logging1;
    logging1.init (path);
    logos::node_config config1 (100, logging1);
    ...
    logos::node_config config2 (50, logging2);
...
TEST (node_config, v1_v2_upgrade)
{
    ...
    tree.put ("receive_minimum", logos::amount (0).to_string_dec ());
...
TEST (node_config, v2_v3_upgrade)
{
    ...
    config1.deserialize_json (upgraded, tree);
    //ASSERT_EQ (logos::uint128_union (0).to_string_dec (), tree.get<std::string> ("inactive_supply"));
...
TEST (node, confirm_locked)
{
    ...
    auto block (std::make_shared<logos::send_block> (0, 0, 0, logos::keypair ().prv, 0, 0));
    logos::transaction transaction (system.nodes[0]->store.environment, nullptr, false);
...
TEST (node, fork_publish)
{
    std::weak_ptr<logos::node> node0;
    {
        ...
        logos::keypair key1;
        logos::genesis genesis;
        auto send1 (std::make_shared<logos::send_block> (genesis.hash (), key1.pub, logos::genesis_amount - 100, logos::test_genesis_key.prv, logos::test_genesis_key.pub, 0));
        node1.work_generate_blocking (*send1);
        logos::keypair key2;
        auto send2 (std::make_shared<logos::send_block> (genesis.hash (), key2.pub, logos::genesis_amount - 100, logos::test_genesis_key.prv, logos::test_genesis_key.pub, 0));
        ...
        logos::transaction transaction (node1.store.environment, nullptr, false);
        election->compute_rep_votes (transaction);
        ASSERT_EQ (2, election->votes.rep_votes.size ());
        node1.process_active (send2);
        node1.block_processor.flush ();
        auto existing1 (election->votes.rep_votes.find (logos::test_genesis_key.pub));
        ...
        ASSERT_EQ (logos::genesis_amount - 100, winner.first);
    }
...
TEST (node, fork_keep)
{
    ...
    auto send1 (std::make_shared<logos::send_block> (genesis.hash (), key1.pub, logos::genesis_amount - 100, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.work.generate (genesis.hash ())));
    auto send2 (std::make_shared<logos::send_block> (genesis.hash (), key2.pub, logos::genesis_amount - 100, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.work.generate (genesis.hash ())));
    node1.process_active (send1);
    node1.block_processor.flush ();
    ...
    {
        logos::transaction transaction0 (system.nodes[0]->store.environment, nullptr, false);
        logos::transaction transaction1 (system.nodes[1]->store.environment, nullptr, false);
...
TEST (node, fork_flip)
{
    ...
    logos::keypair key1;
    logos::genesis genesis;
    std::unique_ptr<logos::send_block> send1 (new logos::send_block (genesis.hash (), key1.pub, logos::genesis_amount - 100, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.work.generate (genesis.hash ())));
    logos::publish publish1;
    publish1.block = std::move (send1);
    logos::keypair key2;
    std::unique_ptr<logos::send_block> send2 (new logos::send_block (genesis.hash (), key2.pub, logos::genesis_amount - 100, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.work.generate (genesis.hash ())));
    logos::publish publish2;
    ...
    {
        logos::transaction transaction (system.nodes[1]->store.environment, nullptr, false);
...
TEST (node, fork_multi_flip)
{
    ...
    logos::publish publish2;
    publish2.block = std::move (send2);
    std::unique_ptr<logos::send_block> send3 (new logos::send_block (publish2.block->hash (), key2.pub, logos::genesis_amount - 100, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.work.generate (publish2.block->hash ())));
    logos::publish publish3;
...
{
    logos::system system0 (24000, 1);
    logos::system system1 (24001, 1);
    auto & node1 (*system0.nodes[0]);
    auto & node2 (*system1.nodes[0]);
    system0.wallet (0)->insert_adhoc (logos::test_genesis_key.prv);
    logos::block_hash latest (system0.nodes[0]->latest (logos::test_genesis_key.pub));
    logos::keypair key1;
    auto send1 (std::make_shared<logos::send_block> (latest, key1.pub, logos::genesis_amount - logos::Gxrb_ratio, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system0.work.generate (latest)));
    logos::keypair key2;
    auto send2 (std::make_shared<logos::send_block> (latest, key2.pub, logos::genesis_amount - logos::Gxrb_ratio, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system0.work.generate (latest)));
...
TEST (node, fork_open)
{
    ...
    logos::keypair key1;
    logos::genesis genesis;
    std::unique_ptr<logos::send_block> send1 (new logos::send_block (genesis.hash (), key1.pub, 0, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.work.generate (genesis.hash ())));
    logos::publish publish1;
    publish1.block = std::move (send1);
    node1.process_message (publish1, node1.network.endpoint ());
    node1.block_processor.flush ();
    std::unique_ptr<logos::open_block> open1 (new logos::open_block (publish1.block->hash (), 1, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub)));
    logos::publish publish2;
    publish2.block = std::move (open1);
    node1.process_message (publish2, node1.network.endpoint ());
    node1.block_processor.flush ();
    std::unique_ptr<logos::open_block> open2 (new logos::open_block (publish1.block->hash (), 2, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub)));
    logos::publish publish3;
...
TEST (node, fork_open_flip)
{
    ...
    logos::keypair rep1;
    logos::keypair rep2;
    auto send1 (std::make_shared<logos::send_block> (genesis.hash (), key1.pub, logos::genesis_amount - 1, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.work.generate (genesis.hash ())));
    node1.process_active (send1);
    node2.process_active (send1);
    // We should be keeping this block
    auto open1 (std::make_shared<logos::open_block> (send1->hash (), rep1.pub, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub)));
    // This block should be evicted
    auto open2 (std::make_shared<logos::open_block> (send1->hash (), rep2.pub, key1.pub, key1.prv, key1.pub, system.work.generate (key1.pub)));
    ...
    ASSERT_EQ (logos::genesis_amount - 1, winner.first);
...
TEST (node, coherent_observer)
{
    logos::system system (24000, 1);
    auto & node1 (*system.nodes[0]);
    node1.observers.blocks.add ([&node1](std::shared_ptr<logos::block> block_a, logos::account const &, logos::uint128_t const &, bool) {
        logos::transaction transaction (node1.store.environment, nullptr, false);
        ASSERT_TRUE (node1.store.block_exists (transaction, block_a->hash ()));
    });
    system.wallet (0)->insert_adhoc (logos::test_genesis_key.prv);
    logos::keypair key;
    system.wallet (0)->send_action (logos::test_genesis_key.pub, key.pub, 1);
}
...
TEST (node, fork_no_vote_quorum)
{
    ...
    system.wallet (0)->send_action (logos::test_genesis_key.pub, key4, logos::genesis_amount / 4);
    auto key1 (system.wallet (1)->deterministic_insert ());
    system.wallet (1)->store.representative_set (logos::transaction (system.wallet (1)->store.environment, nullptr, true), key1);
    auto block (system.wallet (0)->send_action (logos::test_genesis_key.pub, key1, node1.config.receive_minimum.number ()));
    ...
    logos::send_block send1 (block->hash (), key1, (logos::genesis_amount / 4) - (node1.config.receive_minimum.number () * 2), logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.work.generate (block->hash ()));
    ASSERT_EQ (logos::process_result::progress, node1.process (send1).code);
    ASSERT_EQ (logos::process_result::progress, node2.process (send1).code);
    ASSERT_EQ (logos::process_result::progress, node3.process (send1).code);
    auto key2 (system.wallet (2)->deterministic_insert ());
    auto send2 (std::make_shared<logos::send_block> (block->hash (), key2, (logos::genesis_amount / 4) - (node1.config.receive_minimum.number () * 2), logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.work.generate (block->hash ())));
    logos::raw_key key3;
    ASSERT_FALSE (system.wallet (1)->store.fetch (logos::transaction (system.wallet (1)->store.environment, nullptr, false), key1, key3));
    auto vote (std::make_shared<logos::vote> (key1, key3, 0, send2));
    logos::confirm_ack confirm (vote);
    ...
    while (node3.stats.count (logos::stat::type::message, logos::stat::detail::confirm_ack, logos::stat::dir::in) < 3)
    {
        system.poll ();
    }
    ASSERT_TRUE (node1.latest (logos::test_genesis_key.pub) == send1.hash ());
    ASSERT_TRUE (node2.latest (logos::test_genesis_key.pub) == send1.hash ());
    ASSERT_TRUE (node3.latest (logos::test_genesis_key.pub) == send1.hash ());
}
...
TEST (node, broadcast_elected)
{
    ...
    logos::keypair rep_big;
    logos::keypair rep_small;
    logos::keypair rep_other;
    ...
    {
        logos::transaction transaction0 (node0->store.environment, nullptr, true);
        logos::transaction transaction1 (node1->store.environment, nullptr, true);
        logos::transaction transaction2 (node2->store.environment, nullptr, true);
        logos::send_block fund_big (node0->ledger.latest (transaction0, logos::test_genesis_key.pub), rep_big.pub, logos::Gxrb_ratio * 5, logos::test_genesis_key.prv, logos::test_genesis_key.pub, 0);
        logos::open_block open_big (fund_big.hash (), rep_big.pub, rep_big.pub, rep_big.prv, rep_big.pub, 0);
        logos::send_block fund_small (fund_big.hash (), rep_small.pub, logos::Gxrb_ratio * 2, logos::test_genesis_key.prv, logos::test_genesis_key.pub, 0);
        logos::open_block open_small (fund_small.hash (), rep_small.pub, rep_small.pub, rep_small.prv, rep_small.pub, 0);
        logos::send_block fund_other (fund_small.hash (), rep_other.pub, logos::Gxrb_ratio * 1, logos::test_genesis_key.prv, logos::test_genesis_key.pub, 0);
        logos::open_block open_other (fund_other.hash (), rep_other.pub, rep_other.pub, rep_other.prv, rep_other.pub, 0);
        ...
        ASSERT_EQ (logos::process_result::progress, node0->ledger.process (transaction0, fund_big).code);
        ASSERT_EQ (logos::process_result::progress, node1->ledger.process (transaction1, fund_big).code);
        ASSERT_EQ (logos::process_result::progress, node2->ledger.process (transaction2, fund_big).code);
        ASSERT_EQ (logos::process_result::progress, node0->ledger.process (transaction0, open_big).code);
        ASSERT_EQ (logos::process_result::progress, node1->ledger.process (transaction1, open_big).code);
        ASSERT_EQ (logos::process_result::progress, node2->ledger.process (transaction2, open_big).code);
        ASSERT_EQ (logos::process_result::progress, node0->ledger.process (transaction0, fund_small).code);
        ASSERT_EQ (logos::process_result::progress, node1->ledger.process (transaction1, fund_small).code);
        ASSERT_EQ (logos::process_result::progress, node2->ledger.process (transaction2, fund_small).code);
        ASSERT_EQ (logos::process_result::progress, node0->ledger.process (transaction0, open_small).code);
        ASSERT_EQ (logos::process_result::progress, node1->ledger.process (transaction1, open_small).code);
        ASSERT_EQ (logos::process_result::progress, node2->ledger.process (transaction2, open_small).code);
        ASSERT_EQ (logos::process_result::progress, node0->ledger.process (transaction0, fund_other).code);
        ASSERT_EQ (logos::process_result::progress, node1->ledger.process (transaction1, fund_other).code);
        ASSERT_EQ (logos::process_result::progress, node2->ledger.process (transaction2, fund_other).code);
        ASSERT_EQ (logos::process_result::progress, node0->ledger.process (transaction0, open_other).code);
        ASSERT_EQ (logos::process_result::progress, node1->ledger.process (transaction1, open_other).code);
        ASSERT_EQ (logos::process_result::progress, node2->ledger.process (transaction2, open_other).code);
    }
    ...
    auto fork0 (std::make_shared<logos::send_block> (node2->latest (logos::test_genesis_key.pub), rep_small.pub, 0, logos::test_genesis_key.prv, logos::test_genesis_key.pub, 0));
    node0->work_generate_blocking (*fork0);
    node0->process_active (fork0);
    node1->process_active (fork0);
    auto fork1 (std::make_shared<logos::send_block> (node2->latest (logos::test_genesis_key.pub), rep_big.pub, 0, logos::test_genesis_key.prv, logos::test_genesis_key.pub, 0));
...
TEST (node, rep_self_vote)
{
    ...
    {
        logos::transaction transaction0 (node0->store.environment, nullptr, true);
        logos::send_block fund_big (node0->ledger.latest (transaction0, logos::test_genesis_key.pub), rep_big.pub, logos::uint128_t ("0xb0000000000000000000000000000000"), logos::test_genesis_key.prv, logos::test_genesis_key.pub, 0);
        logos::open_block open_big (fund_big.hash (), rep_big.pub, rep_big.pub, rep_big.prv, rep_big.pub, 0);
        node0->work_generate_blocking (fund_big);
        node0->work_generate_blocking (open_big);
        ASSERT_EQ (logos::process_result::progress, node0->ledger.process (transaction0, fund_big).code);
    ...
    system.wallet (0)->insert_adhoc (rep_big.prv);
    system.wallet (0)->insert_adhoc (logos::test_genesis_key.prv);
    auto block0 (std::make_shared<logos::send_block> (node0->latest (logos::test_genesis_key.pub), rep_big.pub, logos::uint128_t ("0x60000000000000000000000000000000"), logos::test_genesis_key.prv, logos::test_genesis_key.pub, 0));
    node0->work_generate_blocking (*block0);
    ASSERT_EQ (logos::process_result::progress, node0->process (*block0).code);
    auto & active (node0->active);
    active.start (block0);
    auto existing (active.roots.find (block0->root ()));
    ASSERT_NE (active.roots.end (), existing);
    logos::transaction transaction (node0->store.environment, nullptr, false);
    ...
    ASSERT_NE (rep_votes.end (), rep_votes.find (logos::test_genesis_key.pub));
...
TEST (node, DISABLED_bootstrap_no_publish)
{
    ...
    logos::send_block send0 (system0.nodes[0]->latest (logos::test_genesis_key.pub), key0.pub, 500, logos::test_genesis_key.prv, logos::test_genesis_key.pub, 0);
    {
        logos::transaction transaction (node0->store.environment, nullptr, true);
        ASSERT_EQ (logos::process_result::progress, system0.nodes[0]->ledger.process (transaction, send0).code);
    }
...
{
    logos::system system0 (24000, 2);
    system0.wallet (0)->insert_adhoc (logos::test_genesis_key.prv);
    auto node0 (system0.nodes[0]);
    auto node1 (system0.nodes[1]);
    logos::keypair key0;
    logos::send_block send0 (system0.nodes[0]->latest (logos::test_genesis_key.pub), key0.pub, logos::genesis_amount - 500, logos::test_genesis_key.prv, logos::test_genesis_key.pub, 0);
    logos::open_block open0 (send0.hash (), 1, key0.pub, key0.prv, key0.pub, 0);
    logos::open_block open1 (send0.hash (), 2, key0.pub, key0.prv, key0.pub, 0);
    node0->work_generate_blocking (send0);
    ...
    node0->work_generate_blocking (open1);
    {
        ...
        ASSERT_EQ (logos::process_result::progress, node0->ledger.process (transaction0, send0).code);
        ASSERT_EQ (logos::process_result::progress, node1->ledger.process (transaction1, send0).code);
        // They disagree about open0/open1
        ASSERT_EQ (logos::process_result::progress, node0->ledger.process (transaction0, open0).code);
        ASSERT_EQ (logos::process_result::progress, node1->ledger.process (transaction1, open1).code);
    }
    ASSERT_FALSE (node1->bootstrap_initiator.in_progress ());
    node1->bootstrap_initiator.bootstrap (node0->network.endpoint ());
    ASSERT_TRUE (node1->active.roots.empty ());
...
TEST (node, DISABLED_unconfirmed_send)
{
    ...
    wallet0->insert_adhoc (logos::test_genesis_key.prv);
    auto send1 (wallet0->send_action (logos::genesis_account, key0.pub, 2 * logos::Mxrb_ratio));
    auto iterations0 (0);
    while (node1.balance (key0.pub) != 2 * logos::Mxrb_ratio || node1.bootstrap_initiator.in_progress ())
    {
        system.poll ();
        ++iterations0;
    ...
    logos::send_block send2 (latest, logos::genesis_account, logos::Mxrb_ratio, key0.prv, key0.pub, node0.work_generate_blocking (latest));
    {
        logos::transaction transaction (node1.store.environment, nullptr, true);
        ASSERT_EQ (logos::process_result::progress, node1.ledger.process (transaction, send2).code);
    }
    auto send3 (wallet1->send_action (key0.pub, logos::genesis_account, logos::Mxrb_ratio));
    auto iterations (0);
    while (node0.balance (logos::genesis_account) != logos::genesis_amount)
    {
        system.poll ();
        ++iterations;
...
TEST (node, rep_list)
{
    ...
    wallet0->send_action (logos::test_genesis_key.pub, key1.pub, logos::Mxrb_ratio);
...
TEST (node, no_voting)
{
    ...
    ASSERT_EQ (0, node1.stats.count (logos::stat::type::message, logos::stat::detail::confirm_ack, logos::stat::dir::in));
}
...
TEST (node, vote_replay)
{
    logos::system system (24000, 2);
    logos::keypair key;
    auto open (std::make_shared<logos::open_block> (0, 1, key.pub, key.prv, key.pub, 0));
    system.nodes[0]->work_generate_blocking (*open);
    for (auto i (0); i < 11000; ++i)
    {
        logos::transaction transaction (system.nodes[1]->store.environment, nullptr, false);
        auto vote (system.nodes[1]->store.vote_generate (transaction, logos::test_genesis_key.pub, logos::test_genesis_key.prv, open));
    }
    {
        logos::transaction transaction (system.nodes[0]->store.environment, nullptr, false);
        std::lock_guard<std::mutex> lock (system.nodes[0]->store.cache_mutex);
        auto vote (system.nodes[0]->store.vote_current (transaction, logos::test_genesis_key.pub));
        ASSERT_EQ (nullptr, vote);
    }
    system.wallet (0)->insert_adhoc (logos::test_genesis_key.prv);
    auto block (system.wallet (0)->send_action (logos::test_genesis_key.pub, key.pub, logos::Gxrb_ratio));
    ASSERT_NE (nullptr, block);
    auto done (false);
    auto iterations (0);
    while (!done)
...
TEST (node, balance_observer)
{
    ...
    logos::keypair key;
    node1.observers.account_balance.add ([&key, &balances](logos::account const & account_a, bool is_pending) {
        if (key.pub == account_a && is_pending)
        {
            balances++;
        }
        else if (logos::test_genesis_key.pub == account_a && !is_pending)
        {
            balances++;
        }
    });
...
TEST (node, stat_counting)
{
    logos::system system (24000, 1);
    auto & node1 (*system.nodes[0]);
    node1.stats.add (logos::stat::type::ledger, logos::stat::dir::in, 1);
    node1.stats.add (logos::stat::type::ledger, logos::stat::dir::in, 5);
    node1.stats.inc (logos::stat::type::ledger, logos::stat::dir::in);
    node1.stats.inc (logos::stat::type::ledger, logos::stat::detail::send, logos::stat::dir::in);
    node1.stats.inc (logos::stat::type::ledger, logos::stat::detail::send, logos::stat::dir::in);
    node1.stats.inc (logos::stat::type::ledger, logos::stat::detail::receive, logos::stat::dir::in);
    ASSERT_EQ (10, node1.stats.count (logos::stat::type::ledger, logos::stat::dir::in));
    ASSERT_EQ (2, node1.stats.count (logos::stat::type::ledger, logos::stat::detail::send, logos::stat::dir::in));
    ASSERT_EQ (1, node1.stats.count (logos::stat::type::ledger, logos::stat::detail::receive, logos::stat::dir::in));
}
...
TEST (node, online_reps)
{
    ...
    system.wallet (0)->send_action (logos::test_genesis_key.pub, logos::test_genesis_key.pub, logos::Gxrb_ratio);
    auto iterations (0);
...
TEST (node, block_confirm)
{
    ...
    system.wallet (0)->insert_adhoc (logos::test_genesis_key.prv);
    auto send1 (std::make_shared<logos::state_block> (logos::test_genesis_key.pub, genesis.hash (), logos::test_genesis_key.pub, logos::genesis_amount - logos::Gxrb_ratio, logos::test_genesis_key.pub, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.nodes[0]->work_generate_blocking (genesis.hash ())));
    {
        logos::transaction transaction (system.nodes[0]->store.environment, nullptr, true);
        ASSERT_EQ (logos::process_result::progress, system.nodes[0]->ledger.process (transaction, *send1).code);
    }
...
TEST (node, block_arrival_size)
{
    logos::system system (24000, 1);
    auto & node (*system.nodes[0]);
    auto time (std::chrono::steady_clock::now () - logos::block_arrival::arrival_time_min - std::chrono::seconds (5));
    logos::block_hash hash (0);
    for (auto i (0); i < logos::block_arrival::arrival_size_min * 2; ++i)
    {
        node.block_arrival.arrival.insert (logos::block_arrival_info{ time, hash });
        ++hash.qwords[0];
    }
    ASSERT_EQ (logos::block_arrival::arrival_size_min * 2, node.block_arrival.arrival.size ());
    node.block_arrival.recent (0);
    ASSERT_EQ (logos::block_arrival::arrival_size_min, node.block_arrival.arrival.size ());
}
...
TEST (node, confirm_quorum)
{
    ...
    auto send1 (std::make_shared<logos::state_block> (logos::test_genesis_key.pub, genesis.hash (), logos::test_genesis_key.pub, logos::Gxrb_ratio, logos::test_genesis_key.pub, logos::test_genesis_key.prv, logos::test_genesis_key.pub, system.nodes[0]->work_generate_blocking (genesis.hash ())));
    {
        logos::transaction transaction (system.nodes[0]->store.environment, nullptr, true);
        ASSERT_EQ (logos::process_result::progress, system.nodes[0]->ledger.process (transaction, *send1).code);
    }
    ...
    ASSERT_EQ (0, system.nodes[0]->balance (logos::test_genesis_key.pub));
}
#endif
