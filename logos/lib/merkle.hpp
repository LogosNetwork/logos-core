...
/// MerkleRoot calculation helper function
/// @param iterator is the call back that provides iteration over ordered hashes
///     it provides the next hash via repetetive calls to the nested function
/// @returns Merkle tree root
logos::block_hash
MerkleHelper(function<void( function<void(const logos::block_hash&)> )> iterator);