...
BatchBlockConsensusManager::SendBufferedBlocks()
{
    ...
    {
        OnSendRequest(
          static_pointer_cast<RequestMessage<ConsensusType::BatchStateBlock>>(
...
BatchBlockConsensusManager::QueueRequest(
  std::shared_ptr<RequestMessage<ConsensusType::BatchStateBlock>> request)
{
  _handler.OnRequest(request);
}

PrePrepareMessage<ConsensusType::BatchStateBlock> & 
BatchBlockConsensusManager::PrePrepareGetNext()
{
  return reinterpret_cast<PrePrepareMessage<ConsensusType::BatchStateBlock>&>(
...
BatchBlockConsensusManager::ApplyUpdates(
  const PrePrepareMessage<ConsensusType::BatchStateBlock> & pre_prepare, 
  uint8_t delegate_id)
{
