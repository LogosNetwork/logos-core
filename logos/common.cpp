...
logos::Request::Request(bool & error,
                        std::basic_streambuf<uint8_t> & stream)
{
    error = read(stream, type);
    if(error)
    {
        return;
    }

    error = read(stream, previous);
    if(error)
    {
        return;
    }

    error = read(stream, next);
}

uint16_t logos::Request::WireSize() const
{
    return sizeof(type) +
           sizeof(previous) +
           sizeof(next);
...
uint8_t logos::Request::StringWireSize(const std::string & s) const
{
