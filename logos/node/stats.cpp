...
bool logos::stat_config::deserialize_json (boost::property_tree::ptree & tree_a)
{
    bool error = false;
...
std::string logos::stat_log_sink::tm_to_string (tm & tm)
{
...
class json_writer : public logos::stat_log_sink
{
...
class file_writer : public logos::stat_log_sink
{
public:
    std::ofstream log;
    std::string filename;
...
logos::stat::stat (logos::stat_config config) :
config (config)
{
}

std::shared_ptr<logos::stat_entry> logos::stat::get_entry (uint32_t key)
{
    return get_entry (key, config.interval, config.capacity);
}

std::shared_ptr<logos::stat_entry> logos::stat::get_entry (uint32_t key, size_t interval, size_t capacity)
{
    std::unique_lock<std::mutex> lock (stat_mutex);
    return get_entry_impl (key, interval, capacity);
}

std::shared_ptr<logos::stat_entry> logos::stat::get_entry_impl (uint32_t key, size_t interval, size_t capacity)
{
    std::shared_ptr<logos::stat_entry> res;
    auto entry = entries.find (key);
    if (entry == entries.end ())
    {
        res = entries.insert (std::make_pair (key, std::make_shared<logos::stat_entry> (capacity, interval))).first->second;
    }
    else
    {
        res = entry->second;
...
std::unique_ptr<logos::stat_log_sink> logos::stat::log_sink_json ()
{
    return std::make_unique<json_writer> ();
}

std::unique_ptr<logos::stat_log_sink> log_sink_file (std::string filename)
{
    return std::make_unique<file_writer> (filename);
}

void logos::stat::log_counters (stat_log_sink & sink)
{
    std::unique_lock<std::mutex> lock (stat_mutex);
    log_counters_impl (sink);
}

void logos::stat::log_counters_impl (stat_log_sink & sink)
{
    sink.begin ();
...
void logos::stat::log_samples (stat_log_sink & sink)
{
    std::unique_lock<std::mutex> lock (stat_mutex);
    log_samples_impl (sink);
}

void logos::stat::log_samples_impl (stat_log_sink & sink)
{
    sink.begin ();
    if (sink.entries () >= config.log_rotation_count)
    {
...
void logos::stat::update (uint32_t key_a, uint64_t value)
{
...
std::string logos::stat::type_to_string (uint32_t key)
{
    auto type = static_cast<stat::type> (key >> 16 & 0x000000ff);
    std::string res;
    switch (type)
    {
        case logos::stat::type::block:
            res = "block";
            break;
        case logos::stat::type::bootstrap:
            res = "bootstrap";
            break;
        case logos::stat::type::error:
            res = "error";
            break;
        case logos::stat::type::ledger:
            res = "ledger";
            break;
        case logos::stat::type::peering:
            res = "peering";
            break;
        case logos::stat::type::rollback:
            res = "rollback";
            break;
        case logos::stat::type::traffic:
            res = "traffic";
            break;
        case logos::stat::type::vote:
            res = "vote";
            break;
        case logos::stat::type::message:
            res = "message";
            break;
    }
    return res;
...
std::string logos::stat::detail_to_string (uint32_t key)
{
    auto detail = static_cast<stat::detail> (key >> 8 & 0x000000ff);
    std::string res;
    switch (detail)
    {
        case logos::stat::detail::all:
            res = "all";
            break;
        case logos::stat::detail::bad_sender:
            res = "bad_sender";
            break;
        case logos::stat::detail::bulk_pull:
            res = "bulk_pull";
            break;
        case logos::stat::detail::bulk_pull_blocks:
            res = "bulk_pull_blocks";
            break;
        case logos::stat::detail::bulk_push:
            res = "bulk_push";
            break;
        ...
        case logos::stat::detail::confirm_ack:
            res = "confirm_ack";
            break;
        case logos::stat::detail::confirm_req:
            res = "confirm_req";
            break;
        case logos::stat::detail::frontier_req:
            res = "frontier_req";
            break;
        case logos::stat::detail::handshake:
            res = "handshake";
            break;
        case logos::stat::detail::initiate:
            res = "initiate";
            break;
        case logos::stat::detail::insufficient_work:
            res = "insufficient_work";
            break;
        case logos::stat::detail::keepalive:
            res = "keepalive";
            break;
        case logos::stat::detail::open:
            res = "open";
            break;
        case logos::stat::detail::publish:
            res = "publish";
            break;
        case logos::stat::detail::receive:
            res = "receive";
            break;
        case logos::stat::detail::republish_vote:
            res = "republish_vote";
            break;
        case logos::stat::detail::send:
            res = "send";
            break;
        case logos::stat::detail::state_block:
            res = "state_block";
            break;
        case logos::stat::detail::vote_valid:
            res = "vote_valid";
            break;
        case logos::stat::detail::vote_replay:
            res = "vote_replay";
            break;
        case logos::stat::detail::vote_invalid:
            res = "vote_invalid";
            break;
    }
    return res;
...
std::string logos::stat::dir_to_string (uint32_t key)
{
    auto dir = static_cast<stat::dir> (key & 0x000000ff);
    std::string res;
    switch (dir)
    {
        case logos::stat::dir::in:
            res = "in";
            break;
        case logos::stat::dir::out:
            res = "out";
            break;
    }
    return res;
