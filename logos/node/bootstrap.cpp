...
void logos::frontier_req_client::received_frontier (boost::system::error_code const & ec, size_t size_a)
{
    if (!ec)
    {
        ...
        auto error1 (logos::read (account_stream, account));
        assert (!error1);
        ...
        auto error2 (logos::read (latest_stream, latest));
        assert (!error2);
        if (count == 0)
        {
            start_time = std::chrono::steady_clock::now ();
...
void logos::bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a)
{
    if (!ec)
    {
        logos::bufferstream stream (connection->receive_buffer.data (), 1 + size_a);
        std::shared_ptr<logos::block> block (logos::deserialize_block (stream));
...
{
    std::unique_ptr<logos::block> block;
    bool finished (false);
    while (block == nullptr && !finished)
    {
        ...
        if (!finished)
        {
            ...
            if (block == nullptr)
            {
                current_target.first = logos::block_hash (0);
            }
            else
            {
                if (connection->node->config.logging.bulk_pull_logging ())
...
void logos::bulk_push_client::push_block (logos::block const & block_a)
{
    auto buffer (std::make_shared<std::vector<uint8_t>> ());
    {
        logos::vectorstream stream (*buffer);
...
bool logos::bootstrap_attempt::request_frontier (std::unique_lock<std::mutex> & lock_a)
{
    auto result (true);
        ...
        if (!logos::message::read_header (type_stream, version_max, version_using, version_min, type, extensions))
        {
            switch (type)
            {
                ...
                case logos::message_type::frontier_req:
                {
                    ...
                    auto this_l (shared_from_this ());
                    boost::asio::async_read (*socket, boost::asio::buffer (receive_buffer.data () + 8, sizeof (logos::uint256_union) + sizeof (uint32_t) + sizeof (uint32_t)), [this_l](boost::system::error_code const & ec, size_t size_a) {
...
void logos::bootstrap_server::receive_bulk_pull_action (boost::system::error_code const & ec, size_t size_a)
{
    if (!ec)
    {
        ...
        logos::bufferstream stream (receive_buffer.data (), 8 + sizeof (logos::uint256_union) + sizeof (logos::uint256_union));
...
void logos::bootstrap_server::receive_frontier_req_action (boost::system::error_code const & ec, size_t size_a)
{
    ...
    {
        std::unique_ptr<logos::frontier_req> request (new logos::frontier_req);
        logos::bufferstream stream (receive_buffer.data (), 8 + sizeof (logos::uint256_union) + sizeof (uint32_t) + sizeof (uint32_t));
        auto error (request->deserialize (stream));
        if (!error)
        {
            if (node->config.logging.bulk_pull_logging ())
...
void logos::bulk_pull_blocks_server::set_params ()
{
    ...
    if (connection->node->config.logging.bulk_pull_logging ())
    {
        ...
        switch (request->mode)
        {
            case logos::bulk_pull_blocks_mode::list_blocks:
                modeName = "list";
                break;
