...
_tx_receiver{config.tx_acceptor_config.tx_acceptors.size() != 0
    ...
    : nullptr},
do_token_test(config_a.consensus_manager_config.local_address == "172.1.1.114")
{
...
// CH added starting logic here instead of inside constructors
    if(do_token_test)
    {
        LOG_INFO(log) << "SCHEDULING TOKEN TEST!!!!" << std::endl;
        alarm.add(std::chrono::steady_clock::now() + std::chrono::seconds(25), [this]()
        {
            TestTokenRequests();
        });
    }
...
void logos::node::TestTokenRequests()
{
    ...
    auto send_request = [this](auto request_ptr,
                               auto & message,
                               auto expected_result)
    {
        ...
        auto result = OnRequest(request_ptr, false);
        if(result.code != expected_result)
        {
            std::cout << "FAIL"
                      << std::endl
                      << "Unexpected result: "
    ...
    char const * create_accounts = R"%%%({
        ...
        "previous": "F6462CC625D04319163A6C13EA31FCB406B914BBBDFCEC303EEEA5033C0CA4C8",
    ...
    issuer.decode_account("lgs_1sibjaeaceh59dh7fefo49narpsoytqac5hafhujum3grnd7qrhbczfy9wx8");

    AccountAddress controller_1;
    controller_1.decode_account("lgs_15p6h3z7dgif1kt8skmdmo8xmobh3xyfzthoden6jqu34t6i4sgtcr4pfj5h");

    AccountAddress controller_2;
    controller_2.decode_account("lgs_1mkqajo9pedc1x764b5y5yzkykcm3h3hx1bumznzhgjqimjpajy9w5qfsis6");
    ...
    issue_adtl.amount -= issue_adtl.amount.number() % TOKEN_RAW;
    ...
    std::cout << "Current token account logos balance: "
              << account.balance.to_string_dec()
              << std::endl;

    auto talb = account.balance;
