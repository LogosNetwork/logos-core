...
pub fn launch_node(
    ...
    let data_dir = tmp_dir.join(format!("Logos_load_test_{}", i));
    match fs::create_dir(&data_dir) {
        Ok(_) => {}
        Err(ref e) if e.kind() == io::ErrorKind::AlreadyExists => {
            let _ = fs::remove_file(data_dir.join("data.ldb"));
        }
        r => r.chain_err(|| "failed to create logos_core data directory")?,
    }
    ...
    let config = json!({
        ...
        "node": {
            ...
            "preconfigured_representatives": [
                "lgs_3e3j5tkog48pnny9dmfzj1r16pg8t1e76dz5tmac6iq689wyjfpiij4txtdo"
            ],
            "inactive_supply": "0",
    ...
    let child = Command::new(logos_core)
        .arg("--data_path")
        .arg(&data_dir)
        .arg("--daemon")
        .spawn_async(&handle)
        .chain_err(|| "failed to spawn logos_core")?;
...
pub fn connect_node<C: Connect>(
    ...
    })).then(|x| x.chain_err(|| "failed to call logos_core RPC"))
            .map(|_| ()),
    ) as _
}
